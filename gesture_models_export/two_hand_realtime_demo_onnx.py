...
class ONNXClassifier:
    ...
    def __init__(self, onnx_path, class_names_path, img_size=224):
        ...
        with open(class_names_path) as f:
            raw = json.load(f)
        # json keys come back as strings; convert to int-indexed list
...
def main():
    parser = argparse.ArgumentParser(description="ONNX-backed two-hand gesture demo (pre-Unity-handoff test).")
    ...
    print("Camera opened (ONNX backend). Press q or Esc to quit.")
    while True:
        timer.start("capture")
        ok, frame = cap.read()
        if not ok:
        ...
        cv2.imshow("Two Hand Gesture Demo (ONNX)", display_frame)
